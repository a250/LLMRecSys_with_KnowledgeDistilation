...
class ContextModelWrapper(
    ...
    def __init__(
        ...
        super(ContextModelWrapper, self).__init__(model.config, model.dataset)
    ...
    def _get_loss(self, loss_name):
        ...
        if hasattr(losses_module, loss_name):
            print(f'   |                      |> getting loss from models.losses')        
...
#                 out['layers'].append((i, layer) )
    ...
    def substitute_module(self, dummy_layer_n):
        self._travers_model_struc()
        substituted = self.model_struc[dummy_layer_n]
        dum_parent = None
        for k,v in self.model_struc.items():
            if substituted in list(v.children()):
                ind = list(v.children()).index(substituted)
                dum_parent = v
                break
        if dum_parent:
            v[ind] = Nothing()
